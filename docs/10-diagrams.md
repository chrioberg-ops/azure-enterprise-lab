...
Administrator connects to CLIENT01 using Remote Desktop.
CLIENT01 connects internally to DC01.
A domain user signs in.
CLIENT01 sends a DNS request to DC01.
DNS returns the domain controller records.
CLIENT01 sends a Kerberos authentication request.
Active Directory validates the user.
CLIENT01 requests Group Policy.
The Client Security Baseline is applied.
The user requests access to a department file share.
Group membership is checked.
SMB and NTFS permissions are evaluated.
Access is granted or denied.
Heartbeat, Event and Performance data are sent to Azure Monitor.
Purpose
